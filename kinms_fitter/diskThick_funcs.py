...
class diskThick_funcs:
    """
    Creates variable disk thicknesses as a function of radius.
    """
    def __init__(self):
        pass    
    ...
    class linear:  
        """
        Creates a thickness thats linear with radius (optionally: that also flattens after some radius).     
        ...
        """
        def __init__(self,guesses,minimums,maximums,priors=None,precisions=None,fixed=None,labels_prefix='thick'):
            self.guess=np.array(guesses)
            if self.guess.size == 2:
                self.freeparams=2
                self.labels=[labels_prefix+' grad',labels_prefix+' intercept']
                self.units=['arcsec/arcsec','arcsec']
            else:
    ...
    class flat:  
        """
        No disk thickness flat with radius.     
        ...
        fixed: ndarray of bool
            Optional - Fix this parameter to the input value in guesses. [Default = All False]
        """
        def __init__(self,guesses,minimums,maximums,priors=None,precisions=None,fixed=None,labels=['diskThick'],units=['arcsec']):
            self.guess=np.array([guesses])
    ...
    class tilted_rings:  
        """
        Arbitary thickness using the tilted ring formalism. 
        ...
        fixed: ndarray of bool
            Optional - Fix this parameter to the input value in guesses. [Default = All False]
        """
        def __init__(self,bincentroids,guesses,minimums,maximums,priors=None,precisions=None,fixed=None,labels_prefix='thick'):
            self.bincentroids=np.array(bincentroids)
            self.freeparams=self.bincentroids.size
            self.operation="add"
            self.labels=np.array([])
            self.units=[]
            for i in range(0,self.freeparams):
                self.labels=np.append(self.labels,labels_prefix+str(i))
                self.units.append('arcsec')
            self.min=np.array(minimums)
            self.max=np.array(maximums)
            self.guess=np.array(guesses)
